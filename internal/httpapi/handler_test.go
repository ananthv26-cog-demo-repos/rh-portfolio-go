...
func TestUnicodeDecimalMarkHandling(t *testing.T) {
	price, _ := money.Parse("19.990000")
	handler := NewHandler(&store.MemoryStore{Positions: map[string][]domain.Lot{
		"acct-p1\x00HOOD": {{Quantity: 10, Price: price}},
	}})
	tests := []struct {
		...
		{"fullwidth digits", "２０", http.StatusOK, `"unrealized_pnl":"0.10"`},
		{"circled number rejected", "①", http.StatusInternalServerError, ""},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			request := httptest.NewRequest(http.MethodGet,
...
func TestQueryFieldLimit(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "over limit valid path",
			path:   "/v1/portfolio/acct-p1/HOOD/?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodGet,
			status: http.StatusInternalServerError,
		},
		{
			name:   "over limit slashless path redirects",
			path:   "/v1/portfolio/acct-p1/HOOD?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodGet,
			status: http.StatusMovedPermanently,
		},
		{
			name:   "over limit unknown symbol",
			path:   "/v1/portfolio/acct-p1/NOPE/?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodGet,
			status: http.StatusInternalServerError,
		},
		{
			name:   "over limit unknown account",
			path:   "/v1/portfolio/nope-acct/HOOD/?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodGet,
			status: http.StatusInternalServerError,
		},
		{
			name:   "over limit post",
			path:   "/v1/portfolio/acct-p1/HOOD/?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodPost,
			status: http.StatusInternalServerError,
		},
		{
			name:   "over limit head",
			path:   "/v1/portfolio/acct-p1/HOOD/?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodHead,
			status: http.StatusInternalServerError,
		},
		{
			name:   "over limit options",
			path:   "/v1/portfolio/acct-p1/HOOD/?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodOptions,
			status: http.StatusInternalServerError,
		},
		{
			name:   "over limit unmatched path slash",
			path:   "/v1/portfolio/a/b/c/?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodGet,
			status: http.StatusNotFound,
		},
		{
			name:   "over limit unmatched path slashless",
			path:   "/v1/portfolio/a/b/c?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodGet,
			status: http.StatusNotFound,
		},
		{
			name:   "over limit portfolio prefix",
			path:   "/v1/portfolio/?mark=20.00" + strings.Repeat("&", 1000),
			method: http.MethodGet,
			status: http.StatusNotFound,
		},
	}
